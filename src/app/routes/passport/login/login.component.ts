...
export class UserLoginComponent implements OnDestroy,OnInit {
  ...
  init(){
    ...
    const aeskeyEncry = this.rsaServerProvider.encrypt(aeskey);
    this.srv.set("aeskey",aeskey);
    this.loginService.init(aeskeyEncry).subscribe((res)=>{
      console.log(res);
      console.log('初始化成功。');
      this.srv.set("clientId",res.clientId);
    }, (err) => {
      console.log('初始化失败，错误如下：');
      console.log(err);
  ...
  submit() {
    ...
    // setTimeout(() => {
      ...
      if (this.type === 0) {
        ...
        let useInfo =  this.userName.value + '####' + this.password.value;
        const aeskey =this.srv.getNone("aeskey");
        const useInfoAES=this.aesServerProvider.encrypt(useInfo, aeskey)
        console.log("aesKey: "+aeskey);
        console.log("useInfoAES: "+useInfoAES);

        useInfo =  useInfoAES+ '####'
            + this.srv.getNone('clientId');
